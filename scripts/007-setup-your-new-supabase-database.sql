-- Setup database for your new Supabase project: wmbksxmtxbvlvkftaxyl
...
CREATE TABLE barcode_jobs (
  ...
  display_barcode_data BOOLEAN DEFAULT true,
...
-- Insert sample barcode items for QR code jobs
INSERT INTO barcode_items (job_id, sku, data, top_text, bottom_text) 
SELECT 
    j.id,
    'SKU-' || LPAD(s.num::text, 4, '0'),
    'PROD' || LPAD(s.num::text, 6, '0'),
    'Electronics Product',
    '$29.99'
FROM barcode_jobs j
